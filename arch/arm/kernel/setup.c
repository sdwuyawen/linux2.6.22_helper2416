...
int cpu_architecture(void)
{
	...
	} else if ((processor_id & 0x00080000) == 0x00000000) {	/* 0x411 2 9200 和 0x410 6 9265 都进入这里 */
		cpu_arch = (processor_id >> 16) & 7;
		if (cpu_arch)
			cpu_arch += CPU_ARCH_ARMv3;					/* 3,CPU_ARCH_ARMv4T; 7,CPU_ARCH_ARMv5TEJ */
	} else {
		/* the revised CPUID */
...
static void __init setup_processor(void)
{
	 ...
	 */
	list = lookup_processor_type(processor_id);	/* 根据processor_id变量的值，获取__arm920_proc_info结构体的地址，结构体类型是proc_info_list，在include/asm-arm/procinfo.h中定义 */
	if (!list) {
...
void __init setup_arch(char **cmdline_p)
{
	...
	if (mdesc->boot_params)	/* 如果定义了bootloader传入参数的地址，这个地址在mach-smdk2440.c中.boot_params定义 */
		tags = phys_to_virt(mdesc->boot_params);	/* tag列表首地址转换为虚拟地址,0x30000100->0xc0000100 */
	...
	if (tags->hdr.tag == ATAG_CORE) {
		if (meminfo.nr_banks != 0)		/* 如果内核中定义了meminfo结构 */
	...
	parse_cmdline(cmdline_p, from);	/* 对命令行进行早期的处理 */
