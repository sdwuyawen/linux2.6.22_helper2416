...
{	.name = "WM8753 HiFi",
	...
	.capture = { /* dummy for fast DAI switching */		/* snd_soc_pcm_stream */
		.stream_name = "Capture",
		.channels_min = 1,
		.channels_max = 2,
		.rates = WM8753_RATES,
	...
	.dai_ops = {									/* snd_soc_codec_ops */
		.digital_mute = wm8753_mute,				/* 数字静音 */
		.set_fmt = wm8753_mode1h_set_dai_fmt,	/* 格式设置 */
		.set_clkdiv = wm8753_set_dai_clkdiv,		/* 时钟设置 */
		.set_pll = wm8753_set_dai_pll,				/* 时钟设置 */
		.set_sysclk = wm8753_set_dai_sysclk,		/* 时钟设置 */
	},
},
/* DAI Voice mode 1 */
{	.name = "WM8753 Voice",
...
static int wm8753_codec_probe(struct i2c_adapter *adap, int addr, int kind)
{
	...
	/* 新增加 */
	printk("wm8753_codec_probe entered. addr=%02x\n", addr);
	/* 新增加 结束*/
...
{
	/* 新增加 */
	printk("wm8753_i2c_attach entered\n");
	/* 新增加 结束*/
...
{
	struct snd_soc_device *socdev = platform_get_drvdata(pdev);	/* 获取的是板驱动smdk2416_wm8753.c里smdk6400_snd_devdata */
	...
	setup = socdev->codec_data;								/* 0x1a,获取I2C的地址，来自smdk2416_wm8753.c里smdk6400_snd_devdata */
