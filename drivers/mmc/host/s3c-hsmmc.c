...
//YX_MODI begin
	...
	ext_CD_int &= 0x2;	/* GPF1 */
#else
	ext_CD_int = readl(S3C2410_GPGDAT);
	ext_CD_int &= 0x1;	/* GPG0 */
#endif
//YX_MODI end
...
static int s3c_hsmmc_probe (struct platform_device *pdev)
{
	...
	if ((plat_data->enabled == 1) && (plat_data->hwport == 1)) {	// YX_MODI changed to port 1 from port 0
...
#if defined(CONFIG_CPU_S3C6410) || defined(CONFIG_CPU_S3C2450) || defined(CONFIG_CPU_S3C2416)
	if ((plat_data->enabled == 1) && (plat_data->hwport == 1)) {		// YX_MODI changed to port 1 from port 0 for card detect 
